...
### 4. Default / Plan / AcceptEdits / Auto 四种工作模式

按 `Shift+Tab` 可以在三种模式间循环切换：

| 模式                    | 行为               | 适用场景                   |
| --------------------- | ---------------- | ---------------------- |
| **Default**（默认）       | 每次写文件前征求确认       | 日常使用，安全                |
| **Plan**（计划）          | 只读，给方案，不改文件      | 大改动前先讨论；不确定文件结构时探索     |
| **AcceptEdits**（接受编辑） | 写文件直接改，其他工具仍征求确认 | 信任内容修改但想控制高风险操作（如网络请求） |
| **Auto**（自动接受）        | 所有操作直接执行，不问      | 你完全信任当前任务，例如批量格式化      |

> 经验法则：**复杂任务先 `/plan` → 确认方向 → 切回 Default 或 AcceptEdits 让它动手。**
